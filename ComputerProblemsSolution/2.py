...
def calc_score(r, c, x_direction, y_direction, list1, list2, scores_all):
    ...
    for noffset in range(-5, 1):
        ...
        for poffset in range(0, 6):
            x, y = r + (poffset + noffset) * x_direction, c + \
                (poffset + noffset) * y_direction
            if (x, y) in list2:
                position.append(2)
            elif (x, y) in list1:
                position.append(1)
            ...
            if temp_shape5 == shape or temp_shape6 == shape:
                if score > max_score[0]:
                    max_score = (score, ((r + (0 + noffset) * x_direction, c + (0 + noffset) * y_direction),
                                         (r + (1 + noffset) * x_direction,
                                          c + (1 + noffset) * y_direction),
                                         (r + (2 + noffset) * x_direction,
                                          c + (2 + noffset) * y_direction),
                                         (r + (3 + noffset) * x_direction,
                                          c + (3 + noffset) * y_direction),
...
def evaluation(is_ai):
    ...
    for l1 in list1:
        r, c = l1[0], l1[1]
        temp_score, Active_scores_all = calc_score(
            r, c, 0, 1, list1, list2, Active_scores_all)
        Active_score += temp_score
        temp_score, Active_scores_all = calc_score(
            r, c, 1, 0, list1, list2, Active_scores_all)
        Active_score += temp_score
        temp_score, Active_scores_all = calc_score(
            r, c, 1, 1, list1, list2, Active_scores_all)
        Active_score += temp_score
        temp_score, Active_scores_all = calc_score(
            r, c, -1, 1, list1, list2, Active_scores_all)
        Active_score += temp_score
    ...
    for l2 in list2:
        r, c = l2[0], l2[1]
        temp_score, Passive_scores_all = calc_score(
            r, c, 0, 1, list2, list1, Passive_scores_all)
        Passive_score += temp_score
        temp_score, Passive_scores_all = calc_score(
            r, c, 1, 0, list2, list1, Passive_scores_all)
        Passive_score += temp_score
        temp_score, Passive_scores_all = calc_score(
            r, c, 1, 1, list2, list1, Passive_scores_all)
        Passive_score += temp_score
        temp_score, Passive_scores_all = calc_score(
            r, c, -1, 1, list2, list1, Passive_scores_all)
        Passive_score += temp_score
...
def run():
    ...
    while is_game:
        if step_count % 2:
            ...
            piece = Circle(
                Point(GRID_WIDTH * p_ai[0], GRID_WIDTH * p_ai[1]), 16)
            piece.setFill('white')
            piece.draw(gw)
